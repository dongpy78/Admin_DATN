...
import SearchLevel from "../../components/levels/SearchLevel";
import TableLevel from "../../components/levels/TableLevel";
...
export const loader = async ({ request }) => {
  ...
  try {
    const response = await axiosInstance.get(
      `/list-allcodes?type=JOBLEVEL&limit=5&offset=${offset}`
    );
...
export const action = async ({ request }) => {
  ...
  try {
    const searchQuery = searchValues.search
      ...
      : "";
    const url = `/list-allcodes?type=JOBLEVEL&limit=5&offset=${offset}${searchQuery}`;
...
const Level = () => {
  ...
  const fetchTypeJobs = async (page) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message === "No allcodes found for type: JOBLEVEL"
  ...
  const handlePageChange = (pageNumber) => {
    ...
    navigate(`/admin/work-level?${searchParams.toString()}`);
  };
