...
const AddCategoryBlog = () => {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      if (id) {
        ...
        setIsLoading(true);
        try {
          const res = await getCategoryBlogById(id);
  ...
  const handleSaveCategoryBlog = async () => {
    ...
    try {
      let res;
      if (isActionADD) {
        res = await createCategoryBlog({
          name: inputValues.name,
          description: inputValues.description,
        });
      } else {
        res = await updateCategoryBlog(id, {
          name: inputValues.name,
          description: inputValues.description,
        });
      }
      ...
      if (res) {
        ...
        navigate("/admin/categories-blog");
      } else {
  ...
  console.log("Render with inputValues:", inputValues);
