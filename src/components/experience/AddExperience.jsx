...
export const action = async ({ request }) => {
  ...
  try {
    const response = await axiosInstance.post("/create-new-allcode", {
      code: data.code,
      type: "EXPTYPE", // Cố định cho Type Job
      value: data.value,
      image: data.image || "", // Để trống nếu không có
    });

    if (response.status === 200 || response.status === 201) {
      showSuccessToast("Type experience created successfully!");
      return { success: true };
    } else {
...
const AddExperience = () => {
  ...
  if (actionData?.success) {
    setTimeout(() => navigate("/admin/work-exp"), 1000); // Chuyển về danh sách sau 1 giây
  }
  return (
    <Wrapper>
      <Form method="post" className="form">
        <h4 className="form-title">thêm mới kinh nghiệm</h4>
