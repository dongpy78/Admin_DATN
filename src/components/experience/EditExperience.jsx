...
export const loader = async ({ params }) => {
  try {
    const response = await axiosInstance.get(
      `/list-allcodes?type=EXPTYPE&limit=10&offset=0`
    );
    const typeJob = response.data.data.rows.find(
      (tj) => tj.code === params.code
    );
    if (!typeJob) {
      showErrorToast("Type experience not found.");
      return redirect("/admin/work-exp");
    }
    return typeJob;
  } catch (error) {
    showErrorToast(
...
export const action = async ({ request, params }) => {
  ...
  const data = {
    ...
    type: "EXPTYPE", // Cố định theo dữ liệu test
  ...
  try {
    ...
    showSuccessToast("Type experience updated successfully!");
    return redirect("/admin/work-exp");
  } catch (error) {
  ...
  return (
    <Wrapper>
      <Form method="post" className="form">
        <h4 className="form-title">chỉnh sửa kinh nghiệm làm việc</h4>
